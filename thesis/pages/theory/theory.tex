...
A \underline{dominating set} is a subset $D$ of vertices such that all vertices not contained in are adjacent to some vertex in $D$.
The \underline{chromatic number}, $\xi(G)$, of a graph \G is the smallest number of colors, such that adjacent vertices in $V$ are colored differently.

\end{definition}

\begin{graphclass}[Perfect graph]
If the chromatic number equals the size of the maximum clique, \G is called \underline{perfect}. 
They are a superclass for many well-known other graph classes.
\end{graphclass}
...
%The constant is very large and we were able to give a kernel with reasonably small constants.  % Why is the constant large
...
\resizebox{1.0\textwidth}{!}{
    ...
    \begin{tabularx}{1.5\textwidth}{lllllll}
        ...
        bipartite                             & \NPcs~\cite{Bertossi1984}                               & \WTWOhs~\cite{Raman2008}                                      & \NPcs~\cite{Henning2019}                                & \WTWOhs \cref{lemma:bipartite}             & \NPcs~\cite{Pfaff1983}                       & ?                             \\
       ...
       block                        &                                                      \multicolumn{2}{c}{\Ptt~\cite{Farber1984} }                                          & \multicolumn{2}{c}{\Ptt~\cite{Henning2022}}              & \multicolumn{2}{c}{\Ptt~\cite{Chang1989}}                                                                       \\
        ...
        \bottomrule
    \end{tabularx}
    \footnotetext[1]{Galby et al. \cite{Galby2020} attempted it, but found a mistake in their reduction.}
}
\end{minipage}
    \end{table}
\end{center}
