...
\begin{proof}
    \begin{enumerate}
        \item[$\Rightarrow$] Let $D$ be a semitotal dominating set in $G$ of size $k$. 
        ...
        \item[$\Leftarrow$]  Assume $D'$ to be a semitotal dominating set in $G'$. We can assume that $v \in D'$, because $v'$ has to be dominated and it is always better to choose $v$ instead of $v$.
...
\end{fact}
\begin{proof}
Because $\Dvw = \emptyset$, any SDS of $G$ has to contain $v$ or $w$, or at least four vertices from $N_{2,3}(v,w)$. In the second case, these four vertices can be replaced with $v$, $w$ and two neighbors of $v$ and $w$ still forming a semitotal dominating set.
\end{proof}
...
\begin{proof}
    ...
    \begin{enumerate}
        \item[$\Rightarrow$] Consider a semitotal dominating set $D$ in $G$. We show that $G'$ also has a semitotal dominating set with $\abs{D'} \leq \abs{D}$. By assumption, we have $\Dvw = \emptyset$.
        \begin{enumerate}
        ...
        \begin{enumerate}
            ...
            \item  $\Dv = \emptyset \wedge \Dw \neq \emptyset$: Symmetrical to previous case.
        \end{enumerate} 
        In all cases, we have shown that $\abs{D} \leq \abs{D'}$ and $D$ is a semitotal dominating set of $G$.
    \end{enumerate}
...
\begin{proof}
    ...
    \begin{enumerate}
        ...
        Consider a semitotal dominating set $D$ in $G$. We show that $G'$ also has a semitotal dominating set with $\abs{D'} \leq \abs{D}$. By assumption, we have $\abs{V(R) \setminus \{v, w\}} \geq 5$, $R$ is a simple region and therefor $d(v,w) \leq 2$.
        ...
        \item[$\Leftarrow$] Consider a semitotal dominating set $D$ in $G'$. We show that $G$ has a semitotal dominating set with $\abs{D} \leq \abs{D'}$. By assumption, we have $\Dvw = \emptyset$. We analyze both cases separately.
         ...
         Hence \textit{at least one of} $\{v,w \}$ must be in a semitotal dominating set simulating an ``OR-gadget''. Therefore, also $V(R \partial R)$ is dominated by either $v$ or $w$. All the witnesses stayed the same and hence, we simply set $D = D'$.
...
\begin{proof}
    ...
    Let $D$ be a semitotal dominating set, $\mathfrak{R}$ be a maximal \dreg and $v \in D$. Since $D$ dominates all vertices in the graph, we can consider $V$ as $\bigcup_{v \in D}N(v)$ and thus, we only need to bound the sizes of $N_1(v)\setminus \VR$, $N_2(v) \setminus \VR$ and $N_3(v) \setminus \VR$ separately.
...
\begin{proof}
    ...
    As none of our rules change the size of a possible solution $D' \subseteq V'$ in $G'$, we know by \cref{lemma:correctnessone}, \cref{lemma:correctnesstwo} and \cref{lemma:correctnessthree} that $G'$ has a \sdom of size $k$ if and only if $G$ has a semitotal dominating set of size $k$.
    ...
    \end{equation}
    \noindent If $\abs{V(G')} > \kernelsize \cdot k$ $G$ we replace $G'$ by one single vertex $v$, which is trivially a \emph{no}-instance, because $v$ has no witness to form a semitotal dominating set.
