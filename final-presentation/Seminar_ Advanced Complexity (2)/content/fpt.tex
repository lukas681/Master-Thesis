...
%\textbf{Idea: } Distribute a Clique to k partitions s.t. 
...
\begin{center}
  ...
  \begin{tikzpicture}[level/.style={sibling distance=15mm/#1}, scale = 0.8,h]
...
\begin{center}
  ...
  child {node [circle,draw] (w) {$\lor$}
        child {node [] (d) {$x_3$}
        }
  }
  child {node [circle,draw] (q) {$\lor$}
        child {node [] (e) {$x_4$}
        }
  }
  child {node [circle,draw] (r) {$\lor$}
        child {node [] (f) {$x_5$}
        }
  };
  \draw (a) -- (c);
