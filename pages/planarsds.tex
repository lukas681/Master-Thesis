...
\begin{figure}[!ht]
    ...
    \end{equation*}
    \caption[$N_i(v,w)$]{\textit{The neighborhood of a pair of vertices. N_3(v,w)
    
    Furthermore, note that $z \in N_1(w)$, because there is an edge to $v$, but $z \notin N_1(v,w)$ and $z \in N_3(v,w)$}}
...
% TODO Why can that be done?
All of the given reduction rules are local and only need the neighborhood of at most two vertices and replace them partially with gadgets of constant size. Now checking whether a graph after applying the rule has been isomorphically changed can be trivially accomplished in constant time.
