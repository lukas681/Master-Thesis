...
\begin{abstract}
{\sffamily
For a graph \G, a set $D$ is called a \textit{semitotal dominating set}, if $D$ is a dominating set and every vertex $v \in D$ is within distance two to another witness $v' \in D$. The \msdom problem is to find a semitotal dominating set of minimum cardinality. The semitotal domination number $\gamma_{t2}(G)$ is the minimum cardinality of a semitotal dominating set and is squeezed between the domination number $\gamma(G)$ and the total domination number $\gamma_t(G)$. Given a graph \G and a positive integer $k$, the \sdomd problem asks if $G$ has a semitotal dominating set of size at most $k$.
