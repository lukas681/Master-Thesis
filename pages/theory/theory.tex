...
For a specific dominating set $D$, we say that a vertex $d$ is a \textit{dominating vertex} or \textit{dominator} if $d \in D$. We say that $d$ \textit{dominates} all of its neighbors.

The \dom problem asks for a subset $D$ of size at most $k$ of vertices whose set of neighbors covers all the other vertices.
...
By definition, there is always another neighboring dominating vertex for a total dominating set $D$.
Therefore, it is natural to ask what happens, if we restrict the distance to at most two, which leads us straight to the idea of \sdomin.
...
\Cref{fig:dsexamples} shows an examples where the on
