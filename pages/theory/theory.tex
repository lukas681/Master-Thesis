...
\begin{definition}[Domination Parameters]
   ...
   The \underline{total domination number} is the minimum cardinality of a total dominating set of $G$, denoted by $\gamma_t(G)$.
   The \underline{semitotal domination number} is the minimum cardinality of an semitotal dominating set of $G$, denoted by $\gamma_t(G)$
\end{definition}

% Henning?


Since every total dominating set is also an semitotal domianting set and every semitotal dominating set is also a dominating set , we have the following fact first observed by Goddard and Henning \cite{Goddard2014}. 
