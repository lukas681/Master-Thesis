...
\begin{prb}[\MakeUppercase{\SDOM}~{\cite{Goddard2014}}]{prb:tds}
    \begin{tabularx}{1.0\textwidth}{>{\hsize=0.30\hsize}X>{\hsize=0.8\hsize}X}
        \textbf{Input} & Graph \G, $k \in \mathbb{N}$\\
...
Goddard et al. \cite{Goddard2014} first showed that \sdoms is \NPc in the general case and initiated further analysis.  
This section complements the complexity status of \sdom first surveyed by Galby et al. \cite{Galby2020} with the latest results.
Furthermore, we have added another column showing the parameterized complexity. 

\cref{tab:complexities} shows the currently known complexities for \DOM, \SDOM and \TDOM when restricted to specific graph classes next to each other and \cref{fig:bigpicture} visualizes the big picture.

A polynomial-time algorithm has been shown for graphs of bounded \textit{mim}-width by Galby et al. \cite{Galby2020}.
Because decomposition trees of constant \textit{mim}-width can be found for permutation, convex, interval, (circular k-) trapezoid, circular permutation, Dilworth-$k$, $k$-polygon, circular arc, complements of $d$-degenerate, bipartite permutation and convex bipartite graphs in polynomial time \cite{Belmonte2011}, this algorithm immediately implies the existence of a polynomial time algorithm for these graph classes as well. 
...
This thesis does the first steps in approaching the problem from the perspective of \textit{Parameterized Complexity}. 
...
As we do not believe that \sdoms is fundamentally different than \doms, the idea was that also the parameterized complexity mirror for \sdoms. 
We were able to prove \WTWO-hardness for \textit{bipartite} and \textit{split} graphs by giving a graph structure preserving reducing from \doms. On the positive side, we give an explicit construction of a kernel for the \textit{planar} case, which also exists for \doms and \tdoms as well.

It would be interesting to close the remaining complexity gaps.
A discussion about those open problems can be found at the end of this thesis in \cref{ch:closing}.
...
\begin{figure}
    ...
    \caption{\textit{Computational Complexity of \sdom. Graph classes that admit a polynomial time algorithm are marked in \textit{green}, those with an fpt algorithm in \textit{yellow}, those that are not fpt in \textit{red} and those that are unknown are left \textit{white}.
    An additional ``?''  marks those problems, where the classical complexity is also unknown.
    This updates the figure given in the survey by Galby et al. \cite{Galby2020}. Note that we assume the parameterization by solution size.}}
    
    %TODO:Chordal Perfect graph (??), AT-free Polytime sovable, make general and then to perfect, planar circle ..., Check direction circular k tapezoid}}
