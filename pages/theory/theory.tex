...
\begin{prb}[\uppercase{\DOM}~{\cite[p. 586]{Cygan2015}}]{prb:ds}
...
\begin{prb}[\uppercase{\TDOM}~{\cite[p. 596]{Cygan2015}}]{prb:sds}
    \begin{tabularx}{1.0\textwidth}{>{\hsize=0.30\hsize}X>{\hsize=0.8\hsize}X}
        \textbf{Input} & Graph \G, $k \in \mathbb{N}$\\
...
\begin{prb}[\uppercase{\SDOM}~{\cite{Goddard2014}}]{prb:tds}
    \begin{tabularx}{1.0\textwidth}{>{\hsize=0.30\hsize}X>{\hsize=0.8\hsize}X}
        \textbf{Input} & Graph \G, $k \in \mathbb{N}$\\
...
We were able to prove \WTWO-hardness for \textit{bipartite} and \textit{split} graphs by giving a graph structure preserving reducing from \doms-

on the positive side, we were able to give an explicit construction of a kernel for the \textit{planar} case, which also exists for \doms and \tdoms as well.
...
\begin{figure}
    ...
    \caption{\textit{Computational Complexity of \sdom. Graph classes that admit a polynomial time algorithm are marked in \textit{green}, those with an fpt algorithm in \textit{yellow} and those that are not fpt in \textit{red}. If the parameterized complexity is unknown, no color is used. This updates the figure given in the survey by Galby et al. \cite{Galby2020}.}}
