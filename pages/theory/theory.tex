...
\begin{definition}[Subgraph and Induced Subgraph]
    ...
    If $G$ is a subgraph of $G'$ and $G'$ contains all the edges fo $G$ with both endpoints in $V(G')$, then $G'$ is an \underline{induced subgraph} of $G$ and we write $G' = G[V(G')]$.
\end{definition}


%TODO Quote
\begin{definition}[Degrees]
    The \textit{degree} $d_G(v)$ (shortly $d(v)$ if $g$ is clear from the context) of a vertex $v$ is the number of neighbors of v. We call a vertex of degree 0 as \underline{isolated} and one of degree 1 as a \underline{pendant}.
\end{definition}
