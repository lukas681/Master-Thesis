...
The second rule is the heart of the whole reduction and tries to minimize the neighborhood of two distinct vertices This rule follows Garnero and Sau's approach in one of the early versions of \cite{Garnero2014} for \tdom. Interestingly, the reduction rules given in the latest version were not directly transferable to \sdom, because they heavily rely on properties only a \tdom exposes and can not be generalized to the more relaxed \sdom.
...
Before we give the concrete reduction rule, we will define the following three sets. The intuition behind them. 
% TODO

It can be observed that in the worst case four vertices are needed to dominate $N_3(v,w)$ of two vertices $v,w \in V$: $v$, $w$ and two witnesses for them. 

If $d(v,w) \leq 2$, it might even suffice to consider sets of size three, because then there would be an intermediate vertex that could witness $v$ and $w$ at the same time. We believe that the kernel can be improved by applying \cref{rgl:rtwo} only on those distanced two vertices. 

For an example observe the graph consisting of two distinct $K_{1,m}$ with $m \in \mathcal{N}$ with centers $v$ and $w$.

%% Assuming connected, only 3!
