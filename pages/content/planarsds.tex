...
\begin{proof}
    \begin{enumerate}
        ...
        In the first case this vertex in $N_{2,3}(v) \cap D$ could have possibly been a witness for $v$, so we select $v' \in D'$ as a witness for $v$ ensuring $D'$ to be an sds. In both cases, $v'$ is by assumption dominated by $v$ and  $\abs{D} \leq \abs{D'}$.

        \item[$\Leftarrow$]  Assume $D'$ to be an sds in $G'$. We can assume that $v \in D'$, because $v'$ has to be dominated and it is always better to choose $v$ instead of $v$.
...
\begin{proof}
    ...
    \begin{enumerate}
        \item[$\Rightarrow$] Consider am sds $D$ in $G$. 
        We show that $G'$ also has a sds with $\abs{D'} \leq \abs{D}$. By assumption, we have $\Dvw = \emptyset$.
        \begin{enumerate}
