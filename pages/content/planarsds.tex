...
\begin{proof}
    ...
    \begin{enumerate}
        \item[$\Rightarrow$] Consider a \sdom $D$ in $G$. We show that $G'$ also has an SDS with $\abs{D'} \leq \abs{D}$. By assumption, we have $\Dvw = \emptyset$ and $\abs{V(R) \setminus \{v, w\}} \geq 5$. 
        We can safely assume that the border $\partial R$ contains exactly two vertices.
        If $\abs{\partial R}  < 2$, the region would not have vertices strictly inside, because we are missing two boundary paths that enclose these vertices.
        Hence, at least three vertices must lie inside.

        On the other side, we only need to observe those cases, where both $v,w \notin D$. Again, if a vertex $v' \in V(R \setminus \partial R)$ together with $v$ or $w$ are in $D$, we can replace them with both $v$ and $w$ which preserves the dominating and witnessing properties. Even if $v'$ was a sole witness for $v$, we know that $d(v,w) \leq 2$ ($R$ being a simple region), and therefore $v$ is a witness for $w$ and vice versa.
...
%         If $\abs{\partial R \setminus \{v,w\} \cap D} = 0$, then $p_2 \in D$ is forced because this is the only way to dominate $V(R \setminus \partial R)$ with one vertex. This case motivated  xxxThis case is depicted in   xxx
         ...
         \textbf{Contrary, assume} $V(R \setminus \partial R) \ncong P_3$. Again, let us denote this induced path as $(p_1,p_2,p_3)$. 


        picture for that Case
