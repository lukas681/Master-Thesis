...
In the first case, we know by fact \cref{fact:f2} that $v$ and $w$ must be in D. Therefore, we introduce two vertices $v'$ and $w'$ in $G'$ and remove $N_{2,3}(v,w)$ as they are dominated by $v$ and $w$. 
But if we remove $N_{2,3}(v,w)$ entirely, we could lose solutions: First, the case that $v$ is a direct witness of $w$ ($d(v,w) = 2$) or that there is one intermediate witness on a path of lenth three from $v$ to $w$ via vertices in $N_{2,3}(v,w)$.

entirely could break certain distance properties of $v$ and $w$ which is, 

%Common neighbor necessary at all?  

% TODO Let's briefly discuss these cases. TODO application diagram
...
\begin{proof}
    ...
    \begin{enumerate}
        \item[$\Rightarrow$] Consider a \sdom $D$ in $G$. We show that $G'$ also has a SDS with $\abs{D'} \leq \abs{D}$
        \begin{enumerate}
            \item  $ \Dv = \emptyset  \wedge \Dw = \emptyset $: By applying \cref{fact:f2} twice, we know that $v, w \in D $.
