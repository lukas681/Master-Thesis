...
\begin{figure}[ht]
    ...
    \end{equation*}
    \caption[Application of \cref{rgl:rtwo}]{\textit{An application of \cref{rgl:rtwo}: (a) $\Dv = \Dw = \emptyset$ \cref{case:c1}, both $v$ and $w$ must be in the sds and we can remove $N_{2,3}$ and add $\{v',w'\}$. Furthermore, we need to preserve a path of length 3 from $v$ to $w$ by adding $\{y,y'\}$ as well. (b) \cref{case:c2} has been applied and the $N_{2,3}(v)$ removed. Observe that those vertices that cross the dotted line are vertices from $N_1(v)$ and not removed.\cref{case:c3} is symmetric.}
    }
    \label{fig:ruleTwo}
\end{figure}
