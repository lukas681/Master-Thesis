...
\begin{proof}
    ...
    \begin{corollary} $G$ has a dominating set (ds) of size $k$ if and only if $G'$ has a semitotal dominating set (sds) of size $k' = k + 2$.
    \end{corollary} 
    First assume a ds in $G$ of size $k$. 
...
Next, we will prove intractability for \textit{split} graphs which directly implies intractability for \textit{chordal} graphs as well.

\begin{theorem}
    \sdom is \WONEhs-hard when restricted to \textit{split} graphs.
\end{theorem}
\begin{proof}
    Given a split graph \G with an independent set $I = \{z_i\}$ and a clique $K = \{k_z\}$ we construct a split graph $G'$ in the following way:

    Given a split graph \G 

    \begin{corollary} $G$ has a a ds of size $k$ if and only if $G'$ has a sds of size $k' = k + 1$.
...
\begin{figure}[ht]
    ...
    \end{equation*}
\caption{\textit{Constructing another split graph $G'$ by increasing the size of the complete graph by one and adding one additional vertex $u$.}}
\end{figure}
...
\end{figure}
    ...
    \begin{corollary}
    G has a Dominating Set of size k iff $G'$ has a sds of size $k+1$
    \end{corollary}
    \begin{subproof}
