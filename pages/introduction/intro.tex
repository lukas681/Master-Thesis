...
Now they are sitting on all the beautiful water lilies refusing to give it back and the desperate ducks have asked for your assistance.
...
%Finding the optimal places for lookout towers, 
You read that both problems are \NPc~\cite{Garey1979,Henning2019} and they are probably very hard so solve.
%After already forshadowed by Gödel \cite{Hartmanis1989} and Nash (CITE), 

The first results of \NPcn were published independently by Cook \cite{Cook1971} in 1971 and Levin in 1973 \cite{Levin1973}. 
Karp \cite{Karp1972} introduced the idea of problem reductions and started to systematically reduce 19 other problems to each other showing that all of them are equally hard to solve: An efficient way to solve one of them will immediately efficiently solve the others as well. Since then, thousands of problems have been found to be \NPc, among them \dom and \sdom. 
Up to today, the question whether these \NPc problems can be solved efficiently is an open problem. 

Even though it is believed that they can't in the general case, there is still hope if additional information about the underlying structure of the problem is known. 
...
Therefore, you have already been forced to assign a duck to one of those leaving you with a kernel of the problem that can not further be reduced.

You now also saw that none of the `quacking'-relations do cross with each other and started to think about how to use this information...
