...
%TODO Add sum here
\begin{lemma}
    Let \G be a graph and D be a set. For any maximal \dreg~$\mathfrak{R}$ on G it holds that $\abs{D} \leq 2 \cdot \abs{\mathfrak{R}}$
\end{lemma}
\begin{proof}\label{lemma:polesBound}
    By definition, any region has exactly two poles $p_1,p_2 \in D$ which could potentially be different for each region $R_i \in \mathfrak{R}$. Hence, $\abs{D} \leq 2 \cdot \abs{\mathfrak{R}}$ holds.
\end{proof}
...
\begin{proof} 
    ...
    \noindent$\mathbf{N_2(v)}$: According to Alber et al. (\cite*{Alber2004}), we know that $N_2(v) \setminus V(\mathfrak{R})$ can be covered by at most $4 \dGR(v)$ simple regionsbetween $v$ and some vertices from $N_1(v)$ on the boundary of $\mathfrak{R}$ where $\dGR(v) = \abs{R(v,w) \in \mathfrak{R}, w \in D}$ is the number of regions in $\mathfrak{R}$ adjacent to $V$. Intuitively, we know that every region $v$ can be a pole of (which it is as $v \in D$) has at most four $N_1(v)$-vertices to which the $N_2(v)$-vertices can be connected. Note that if there had been any intervening $N_3(v)$-vertices, they would have been reduce. Figure \ref{fig:kernelSize} shows an example.
