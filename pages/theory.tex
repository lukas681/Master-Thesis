...
Let \G be a plane graph reduced under \cref*{rgl:rone,rgl:rtwo} and let D be a SDS of G. If R has a maximal D-region decomposition, then $\abs{V \setminus (V(\mathfrak{R}) \cup D)} \leq 144 \abs{D}$
\end{lemma}
...
\noindent Summing up these three upper bounds for each $v \in D$ we obtain the result using Lemma \ref{lemma:numRegions}:

\begin{equation}
    \begin{split}
        \abs{V \setminus V(\mathfrak{R}) \cup D)} &\leq 48 \cdot \abs{\mathfrak{R}} + \abs{D} \\
