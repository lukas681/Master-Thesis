...
\begin{proof} 
    ...
    \noindent$\mathbf{N_3(v)}$: As we know that \cref{rgl:rone} has exhaustively applied, we trivially see that $\abs{N_3(v)} \leq 1$. Hence, $\abs{\bigcup_{v \in D}N_3(v) \setminus V(\mathfrak{R})} \leq \abs{D}$
    ...
    By lemma \ref*{lemma:simpleregionbound} we know that a simple region can only have at least 6 vertices distinct from its poles. Hence, 
...
Summing up these three upper bounds for each $v \in D$ we obtain $V \setminus V(\mathfrak{R}) \leq 48 \abs{\mathfrak{R} + \abs{D}}$
\end{proof}
