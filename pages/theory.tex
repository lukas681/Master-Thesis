...
Figure \ref{fig:maxRegionDecompose} gives an example on how to decompose a graph into a maximal $D-region$ decomposition with a given \sdom of size 3.
...
% TODO: Rewrite:
We are now putting all our pieces together in order to proof our main result: A linear bound on the kernel size. In order to do so, we distinguish between those vertices that are covered by a maximal \dreg and those that are not. In both cases our reduction rules bound the number of vertices to a consant size which means the kernel size does only depend on the number of regions of this decomposition. Lemma \ref{fig:maxRegionDecompose} states that for any solution D, we only have a linear number of regions that cover the whole graph.
...
\begin{lemma}
    \cite*{Alber2004}
    Every vertex in $u \in V \setminus V(\mathfrak{R})$ is either in D or belongs to a set $N_2(v) \cup N_3(v)$.
\end{lemma}
\begin{proof}
...
Let \G be a plane graph reduced under \cref*{rgl:rone,rgl:rtwo} and let D be a SDS of G. If R has a maximal D-region decomposition, then XXX
\end{lemma}

In the following, we define $\dGR(v) = \abs{R(v,w) \in \mathfrak{R}, w \in D}$ to be the number of regions in $\mathfrak{R}$ adjacent to a vertex $v$.
%TODO Add sum here
\begin{corollary}
    Let \G be a graph and D be a set. For any maximal \dreg~$\mathfrak{R}$ on G it holds that $\sum_{v \in D} \dGR(v) \leq 2 \cdot \abs{\mathfrak{R}}$.
\end{corollary}
\begin{proof}\label{lemma:polesBound}
    By definition, we know that each region $\widetilde{R}$ has exactly two poles, thus there can only exist at most two sets from $\dGR(v_i)$ containing $\widetilde{R}$. Hence, the claim follows.
\end{proof}

With some slightly modifications, the proof given in \cite{Garnero2014} will also hold in our case. Note that although asuming the graph to be reduced, the following proof only relies on \cref*{rgl:rone,rgl:rtwo}. It is a very interesting observation that $N_2$
% TODO: Proof that N_Is are distinct
% TODO Provie a worst configuration
\begin{proof} 
    Again, we will follow the proof proposed by Alber et al. \cite[Proposition 2]{Alber2004}. 
   
    The proof does only rely on \cref*{rgl:rone,rgl:rtwo} and we can use the number of vertices in a simple region we have proofen in \ref*{lemma:simpleregionbound}. In particular, we are going to proof that $V \setminus V(\mathfrak{R}) \leq XX \cdot \abs{\mathfrak{R}} + 2 \cdot \abs{D}$. Directly placing in Lemma~\ref{lemma:numRegions} will give as the desired bound.

    Now let $\mathfrak{R}$ be a maximal \dreg and let $v \in D$. Since D dominates $V$ and therefore every non-dominating vertex in the graph has at least one neighbor in D, we can consider $V$ as $\bigcup_{v \in D}N(v)$ and thus, we only need to bound the sizes of $N_1(v)$, $N_2(v)$ and $N_3(v)$  MINUS R seperately. let $v \in D$ in the following:
    ...
    \noindent$\mathbf{N_3(v)}$: As we know that \cref{rgl:rone} has exhaustively applied, we trivially see that $\abs{N_3(v)} \leq 1$ and hence, 

    \[\abs{\bigcup_{v \in D}N_3(v) \setminus V(\mathfrak{R})} \leq \abs{D}\]

    \noindent$\mathbf{N_2(v)}$: According to Alber et al. (\cite*{Alber2004}), we know that $N_2(v) \setminus V(\mathfrak{R})$ can be covered by at most $4 \dGR(v)$ simple regionsbetween $v$ and some vertices from $N_1(v)$ on the boundary of $\mathfrak{R}$. Intuitively, we know that every region $v$ can be a pole of (which it is as $v \in D$) has at most four $N_1(v)$-vertices to which the $N_2(v)$-vertices can be connected. Note that if there had been any intervening $N_3(v)$-vertices, they would have been reduce. Figure \ref{fig:kernelSize} shows an example.
    By lemma \ref*{lemma:simpleregionbound} we know that a simple region can only have at least 6 vertices distinct from its poles and hence, 
    
    \[\abs{\bigcup_{v \ in D} N_2(v) \setminus \mathfrak{R} \leq 6 \sum_{v \in D}4 \cdot \dGR(v) } = 24 \cdot \sum_{v \in D}\dGR(v) \overset{\text{Corollary \ref{lemma:polesBound}}}{\leq} 48 \abs{\mathfrak{R}}\]

    % TODO Now use the lemma in the revised version. 
    % TODO Formally Proof that
    % TODO Make a picture for that case 

    \noindent$\mathbf{N_1(v)}$: By Lemma XX, we know that $N_1(v) \subseteq V(\mathfrak{R})$ and hence, 

    \[\abs{\bigcup_{v \in D}N_1(v) \setminus V(\mathfrak{R})} = 0\]

Summing up these three upper bounds for each $v \in D$ we obtain using lemma \ref{lemma:numRegions} 

\[V \setminus V(\mathfrak{R}) \leq 48 \cdot \abs{\mathfrak{R}} + \abs{D} \leq 48 \cdot (3 \abs{D} - 6) + \abs{D} \leq 144 \abs{D} + \abs{D} = 145 \abs{D}\]
\end{proof}
...
\begin{figure}[!ht]
    ...
    \end{equation*}
\caption{\textit{Bounding the number of $N_2(v)$-vertices around a dominating vertex $v$. $v$ is a pole of $R_1, R_2,...R_j$ and can span simple regions with the help of $N_2(v)$-vertices to at most 4l $N_1(v)$-vertices}. These regions are being reduced by \cref*{rgl:rthree}.}
...
\begin{lemma}[{\cite[Proposition 1]{Garnero2014}}]\label{lemma:numRegions}
