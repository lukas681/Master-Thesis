...
\name{Full-Degree Spanning Tree}\xspace ( in \cite{Guo2006})
...
In an attempt to extend these ideas to other problems as well, Bodlaender et al. (\cite{Bodlaender2016}) proofed that all problems expressivle in counting monadic second-order logic who sastisfy a coverability property admit a polyynomial kernel on graphs of bounded genus g.

Although these results are very interesting from a theoretical point of view, the constants for the kernels obtained by these methods are so large that they are not of practical interest. The question we also follow in this paper is how such a kernel can explicitely and efficiently be constructed.



In order to achieve this, we will first split up the neighborhood of one vertice and a pair of vertices into three distinct subsets, give some nice properties about them, and then state the corresponding reduction rules.  
...
section{The Main Idea and The Big Picture}
 ...
 The main idea is to decompose a graph into so called ``regions'' where we can bound the number of vertices that are left, after some reduction rules have operated on the graph.
...
Again, $N_1(v,w)$ are those connected with the rest of the graph, $N_2(v, w)$ are a \textit{buffer} between $N_3(v,w) \cup \{v, w\}$ and $N_3(v,w)$ are those isolated from the rest of the graph, but can still be useful as a witness for $v$ or $w$. 
...
\cref{fig:maxRegionDecompose} gives an example of how to decompose a graph into a maximal $D-region$ decomposition with a given \sdom of size 3.
...
\begin{lemma}[\#Vertices inside a Region after \cref{rgl:rone,rgl:rtwo,rgl:rthree}]\label{lemma:outside}
...
\begin{proof}
    ...
    \begin{equation}
        139 \cdot (3k - 6) + 145 \cdot k + k \leq 562 \cdot k
    \end{equation}

    \noindent If $\abs{V(G')} > 562 \cdot k$ G is a NO-instance and we replace G' by two single disconnected vertices (trivial NO-instance). Then the kernel is of the claimed size.
