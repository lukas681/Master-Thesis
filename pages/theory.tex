...
\begin{proof} 
    ...
    \noindent$\mathbf{N_2(v)}$: According to Alber et al. (\cite*{Alber2004}), we know that $N_2(v) \setminus V(\mathfrak{R})$ can be covered by at most $4l$ simple regionsbetween $v$ and some vertices from $N_1(v)$ on the boundary of $\mathfrak{R}$ where $l = \abs{R(v,w) \in \mathfrak{R}, w \in D}$ is the number of regions in $\mathfrak{R}$ adjacent to $V$. Intuitively, we know that every region $v$ can be a pole of (which it is as $v \in D$) has at most four $N_1(v)$ vertices to which the $N_2(v)$-vertices can be connected. 
