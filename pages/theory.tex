...
Lemma \ref{fig:maxRegionDecompose} states that for any solution D, we only have a linear number of regions that cover the whole graph. 
...
\begin{lemma}
    Given a plane Graph \G and a region R(v,w) $\abs{N_1(v,w) \cap V(R)} \leq 4$ and these vertices lay exactly on the boundary $\partial R$ of $R$. 
\end{lemma}
\begin{proof}
    TO BE DONE
\end{proof}

\begin{lemma}\cite[See Fact 5]{Garnero2018}
    Given a reduced plane graph \G and a region $R(v,w)$, $N_2(v,w) \cap N$ can be coverd by at most 6 simple regions.
\end{lemma}
\begin{proof}
    ...
    Furthermore, asuming the graph to be reduced, any intermediate $N_3(v,w)$ which could possible seperate multiple simple regions between $v$ and $u_i$ have been deleted by \cref{rgl:rone} already.
\end{proof}
...
The proof does only rely on \cref*{rgl:rone,rgl:rthree} and we can use the number of vertices in a simple region we have proofen in \ref{lemma:simpleregionbound}. In particular, we are going to proof that $V \setminus V(\mathfrak{R}) \leq 48 \cdot \abs{\mathfrak{R}} + 2 \cdot \abs{D}$. Directly placing in Lemma~\ref{lemma:numRegions} will give as the desired bound.
...
Because G is reduced by assumption, we know by lemma \ref{lemma:simpleregionbound} that a simple region can only have at least 6 vertices distinct from its poles and hence,
...
\noindent$\mathbf{N_1(v)}$: By Lemma \ref{lemma:noneinside}, we know that $N_1(v) \subseteq V(\mathfrak{R})$ and hence, 

\[\abs{\bigcup_{v \in D}N_1(v) \setminus V(\mathfrak{R})} = 0\]

\noindent Summing up these three upper bounds for each $v \in D$ we obtain the result using the equation from Lemma \ref{lemma:numRegions}:
