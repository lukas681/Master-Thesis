...
%TODO Add sum here
\begin{lemma}
    Let \G be a graph and D be a set. For any maximal \dreg~$\mathfrak{R}$ on G it holds that $\sum_{d \in D} \leq \abs{\mathfrak{R}}$.
\end{lemma}
...
\begin{proof} 
    ...
    \noindent$\mathbf{N_3(v)}$: The trivial case. As we know that \cref{rgl:rone} has exhaustively applied, we know that $\abs{N_3(v)} \leq 1$. Hence, $\abs{\bigcup_{v \in D}N_3(v) \setminus V(\mathfrak{R})} \leq \abs{D}$
    ...
    By lemma \ref*{lemma:simpleregionbound} we know that a simple region can only have at least 6 vertices distinct from its poles. Hence, $\abs{\bigcup_{v \ in D} N_2(v) \setminus \mathfrak{R} \leq 6 \sum_{v \in D}4 \cdot \dGR(v) } = 24 \cdot \sum_{v \in D}\dGR(v) \leq^{\mathit{Lemma \ref{lemma:polesBound}}} $

    % Now use the lemma in the revised version. 

    % TODO Migrate l to D_Gr(v)
    % TODO Formally Proof that
    % TODO Make a picture for that case 

    \noindent$\mathbf{N_1(v)}$. Extra Lemma why it is the case TODO

Summing up these three bounds, for each $v \in D$ we get XXXXXX
\end{proof}
