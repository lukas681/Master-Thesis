...
\cref{fig:maxRegionDecompose} gives an example on how to decompose a graph into a maximal $D-region$ decomposition with a given \sdom of size 3.
...
\begin{lemma}
    Given a plane Graph \G and a $vw$-region R(v,w) $\abs{N_1(v,w) \cap V(R)} \leq 4$ and these vertices lay exactly on the boundary $\partial R$ of $R$. 
\end{lemma}
\begin{proof}
    TO BE DONE
\end{proof}
...
\begin{proposition}[\#Vertices outside a Region]
