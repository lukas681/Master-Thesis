...
\begin{equ}[!ht]\label{fig:neighborhoodDouble}
    \begin{equation*}
...
Note that for example a vertex $z \in N_1(v)$ is not necessarily also in $z \notin N_1(v,w)$. Figure 
 
\begin{equ}[!ht]\label{fig:neighborhoodWeird}
    \begin{equation*}
        \tikzfig{fig/tikz/neighborhoods-odd}
    \end{equation*}
\caption{\textit{The vertex $z$ is in $N_1(v)$, because there is an edge outside of $N(v)$ to $w$. Contrary, it is not in $N_1(v,w)$, but now  belongs to $N_3(v,w)$, because we are considering the ``shared'' neighborhood}}
\end{equ}
