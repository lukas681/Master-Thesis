...
\begin{theorem}
    \sdom is $\omega[2]$ hard for bipartite Graphs
\end{theorem}
...
\begin{proof}
    ...
    \end{corollary} 
     $\Rightarrow$: Assume there exists a ds $D$ in $G$ with size §k§. $DS = D\cup \{d_1,d_2\}$ is a ds in $G'$ with size $k' = k+2 $ , because $d_1$ dominates $u_1$ and all $x'_i$; $d_2$ dominates $u_2$ and all $y'_i$. Hence, it is an sds, because $\forall v \in (D \cap X): d(v, d_1) = 2$ and $\forall v \in (D \cap Y): d(v, d_2) = 2$

    $\Leftarrow$: On the contrary, assume any sds $SD$ in $G'$ with size $k'$. WLOG we can assume that $u_1, u_2 \notin DS$. 
    
    Our construction forces $d_1$, $d_2 \in DS$. Because all $x'_i$ are only important in dominating $x_i$ ($y'_i$ for $y_i$ resp.) as $d_1, d_2 \in DS$. If $x'_i \in DS$ we simply exchange it with $x_i$ (for $y'_i$ and $y_i$ respectively) in our S keeping the size of the ds. $D = DS \setminus \{ d_1,d_2\}$ give us a ds in $G$ with size $ k = k' - 2$

    %TODO CITE HARDNESS
    As G' can be constructed in $\mathcal{O}(n)$ and parameter k is only blown up by a constant, this reduction is an FPT reduction. As \dom is $w[2]$ hard for bipartite Graphs so is \sdom.
\end{proof}
...
\end{figure}
    ...
    \end{corollary}
    \begin{subproof}
    $\Rightarrow$: Assume $G$ chordal. Then exists a total elemination order $o = (v_1, ..., v_n)$ in G where removing $v_j$ sequentially returns cliques in $N(v_i)$.
    ...
    \end{corollary}
    \begin{subproof}
    Assume a ds $D$ of size $k$ in $G$. $D \cup \{u\}$ is a sds in G' of size k + 1, because $u$ dominates $t$ and for each $v \in DS: d(v, u) \leq 2$.
